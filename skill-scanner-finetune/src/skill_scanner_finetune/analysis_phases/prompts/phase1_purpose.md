...
<!-- TODO: finalize prompt text.
     Design notes:
       - description is an AUDIT TARGET, not ground truth.
       - minimal_purpose MUST be tight: single verb + single object, ≤10 words,
         no conjunctions (and/or/plus), no qualifiers ("while also",
         "that additionally", "optionally").  If the declared purpose cannot
         collapse to that shape, it belongs in purpose_delta instead.
       - 13-category + 9-principle probe runs against description first.
       - fallback chain when description probe hits:
