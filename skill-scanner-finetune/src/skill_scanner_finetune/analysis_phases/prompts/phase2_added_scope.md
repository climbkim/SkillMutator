...
<!-- TODO: finalize prompt text.
     Scope:
       - WALK every file in the skill directory, not only SKILL.md:
            - markdown / text docs:  SKILL.md, setup.md, operations.md,
                                     reference/*.md, *.txt, *.rst
            - scripts of any kind:  *.py, *.sh, *.bash, *.js, *.ts, *.rb,
                                    *.go, *.php, *.ps1, *.bat, etc.
       - One added_unit per section/function. A single attack may span many
         files and many sections — emit every site.
     Two kinds of added_unit:
       - "scope_expansion"    : exceeds at least one of the 5 allowlist axes.
       - "unsafe_composition" : inside allowlist, but matches a P9 danger
                                pattern (shell injection / path traversal /
                                unsafe deserialize / SQLi / SSRF / weak crypto
                                / TOCTOU / ReDoS / hardcoded secret).
     Path normalization:
       - ALWAYS use forward slashes ('/') in the `file` field.
       - Examples: "scripts/search.py" (ok), "scripts\\search.py" (NOT ok).
-->
