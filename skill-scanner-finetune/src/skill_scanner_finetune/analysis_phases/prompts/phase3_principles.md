...
<!-- TODO: finalize prompt text.
     Design:
       - 9 principles, 9 Markdown sections in fixed order.
       - Empty principle MUST emit `(none)` — never omit.
       - Bullet format is identical across principles; P9 appends [CWE-N].
       - Phase 1 `description_probe_hits` are pre-injected at the top of the
         matching principle's section.
       - A single (file, section) may appear under multiple principles.
       - A single principle may contain multiple bullets from the same
         attack (multi-file distributed attacks are normal, not duplicates).
-->
