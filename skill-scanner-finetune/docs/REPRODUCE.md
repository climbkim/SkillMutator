...
This document covers the fine-tuning tables (paper Tables V and VI) and the §6.4 Findings 1–3 from the Fine-tuning Effectiveness section. The benchmark itself — Tables III and IV — is reproduced from the sister repository [`skill-mutator`](https://github.com/ANONYMOUS-ID/skill-mutator).
...
| 68 community-authored skills | Training corpus (see sister repo `docs/SKILLS_SETUP.md`) |
| ~ 80 GB disk | Tokenizer + base model + LoRA checkpoints |

```bash
pip install -e .[all]          # also pulls in skill-mutator
...
`iter_0` carries the attack's semantic structure in its clearest form, and `iter_1` carries the same attack intent re-expressed in scanner-evasion-driven phrasing. `iter_2` samples are a small carry-over from intermediate runs and were retained because they did not regress eval performance; further inclusion of `iter_2` and beyond regressed prefill detection (see `EXPERIMENTS.md` leaderboard).
...
| loss style                | all-token (NOT assistant-only; assistant-only loss was numerically unstable on this dataset size, see EXPERIMENTS.md C21) |
...
# Build paper Table VI (cross-family comparison)
...
Only Qwen benefits from forced Phase 4 prefill; the other three families score better without it (chat-template incompatibility). See paper §6.4 Finding 2.

### 6. Phase ablation (paper Finding 3)

For Finding 3 you re-train with a truncated phase set and re-evaluate:
...
The expected ablation table is produced by `analysis/rq4/scripts/build_tab_phase_ablation.py`; the per-variant `judge_summary.csv` files follow the same schema as Repo A's analysis tooling.

## Notes on inference-time prefill

The training corpus contains `## Phase 1` through `## Phase 4` markers (see `dataset_builder/formatter_v3.py`); inference forces the **same `## Phase 4: Category Mapping` heading** as the assistant continuation, eliminating the chance that small models will skip directly to free-form text. Empirically this is worth +9.2 pp on Qwen, but slightly negative on Llama, Mistral, and Gemma (chat-template incompatibility); see paper §6.4 Finding 2 and Table VI.

> **Heads-up:** earlier development snapshots used a 5-phase variant where the
> assistant continuation was forced to `## Phase 4: Security Findings`. The
> released code path uses the four-phase schema (`P1–P4` with `Phase 4` as the
> prefill target). If you copy code from internal snapshots, verify the
> prefill string matches the schema your dataset was built with.
