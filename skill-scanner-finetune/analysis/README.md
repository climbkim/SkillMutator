...
Paper RQ3 / RQ4 reproduction tables and the v2 GPT-5.4 judge used to
adjudicate scanner outputs.

```
analysis/
├── judge.py             # single-source-of-truth GPT-5.4 v2 judge prompt
├── parsers.py           # scan.md / verdict-json parsers (lifted from sister package)
├── rq3/scripts/
│   └── build_tab_finetune.py        # cross-family detection table (Finding 2)
└── rq4/scripts/
    └── build_tab_phase_ablation.py  # P1 / P12 / P123 / P1234 ablation (Finding 3)
```
...
```bash
# Cross-family (Repo B's RQ3 / paper Finding 2)
python analysis/rq3/scripts/build_tab_finetune.py \
       --root /path/to/Finetune-RQ1

# Phase ablation (paper Finding 3)
python analysis/rq4/scripts/build_tab_phase_ablation.py \
       --root /path/to/Finetune-RQ2
```
